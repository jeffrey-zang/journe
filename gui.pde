...
public void TextBoxChange(GTextField source, GEvent event) { //_CODE_:TextBox:338386:
  ...
  // If press enter button, and the typing textbox is not empty:
  if (event == GEvent.ENTERED && !(TextBox.getText().equals(" "))) {
    // check keywords and add to score from text
    keywordLog.match(TextBox.getText());
    //Add to log to be saved when closed
    ...
    // JournE sends another Prompt message if haven't hit daily quota of 5
    if (questionsAsked < 5) {
      messageLog.addMessage(
        "Bot",
        year(),
    ...
    // Else Give one ending prompt, and stop sending
